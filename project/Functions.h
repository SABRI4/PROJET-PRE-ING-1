...
int verif(int p){
    // Permet de vérifier si l'utilisateur n'as pas fait d'erreurs d'input
    int t = 0;
    while(p !=2 || p!= 1){
        if (t == 0){
            printf("Une erreur de mapping à été détecter, réessayer");
            scanf("%d",&p);
            t+=1;
        }
        else {
...
int defil(const char* filen) {
    ...
    int i = 0;
    char buffer[1000]; // Tampon de lecture
    ...
    while (fgets(buffer, sizeof(buffer), file) != NULL) {
        ...
        while (buffer[i] != '\0') {
            printf("%c", buffer[i]);
            fflush(stdout);
            usleep(DELAY); /* takes a number of microseconds for the break */
            i += 1;
        }
    }
...
void ini_prota(character* p) {
    ...
    scanf("%s", c);
    strcpy(c, p->name);   //On copie le nom dans la structure
    ...
    if (t == '1') {
        ...
        p->power *= 1.15;                                   //La puissance par défaut est modifiée
    }
    else if (t == '3') {
    ...
    fprintf(pl, "%s\n %s\n %s\n %f\n %d\n","100", c, p->eq1, p->power , 0);     //On copie les infos du joueur dans le fichier
...
void save(char* name){
    FILE *pl, *sv;
    remove(name);           //On supprime la sauvegarde si elle existe déjà
    char c;

    
    pl = fopen("perso.txt", "r");       // Ouvre le fichier du joueur 
    if (pl == NULL) {           //On vérifie l'ouverture du fichier joueur
        printf("Error occurred while opening the file.\n");
        exit(1);
    }

    sv = fopen(name, "w");       //Création du fichier de sauvegarde
    while ((c = fgetc(pl)) != EOF) {    //On copie le fichier du joueur dans le fichier sauvegarde
      fputc(c,sv);
   }
    fclose(pl);     //On ferme les 2 fichiers
    fclose(sv);
}
...
character* begin(int *b){
    ...
    char c;
    character *p = NULL;            //On demande si le joueur charge une sauvegarde ou entame une nouvelle partie
    scanf("%c",&c);
    if(c == 'Y'){
